...
################################################################################
function getHomoclinicOrbit(hom::HomoclinicHyperbolicProblemPBC{Tbvp}, x::ArrayPartition, par0; kode...) where {Tbvp <: ShootingProblem}
	sh = hom.bvp
	M = BK.getMeshSize(sh)
	...
	if  M>=1
		...
		return getPeriodicOrbit(hom.bvp, vcat(xflow, T), par; kode...)
	else
...
function initBVPforPBC(bvp0::ShootingProblem, prob_vf, Hom; N, T, ϵ)
	M = BK.getMeshSize(bvp0)
	dt = 2T/M
	xflow = reduce(vcat, [Hom.orbit(-T + n*dt, ϵ) for n = 0:M-1] )
	bvp = @set bvp0.updateSectionEveryStep = 0
...
"""
function generateHomProblem(sh::ShootingProblem,
							x::AbstractArray,
							pars,
							lensHom::Lens;
							verbose = false,
	...
	solpo = BK.getPOSolution(sh, x, pars)

	# find the saddle point as minimum of vector field norm
	xc = BK.getTimeSlices(sh, x)
	ind_saddle = argmin(norm(BK.vf(sh.flow, solpo(t), pars)) for t in time)
	xsaddle = solpo(time[ind_saddle])
	tsaddle = time[ind_saddle]
	solsaddle = BK._newton(BifurcationProblem((x,p) -> BK.vf(sh.flow,x,p),xsaddle,nothing), xsaddle, pars, NewtonPar(verbose = true), norm = x->norm(x,Inf))
	...
	@set! bvp.updateSectionEveryStep = 0
	dt = Thom/M
