...
mutable struct HomoclinicHyperbolicProblemPBC{Tbvp, Nfree, Tlens, Ty, Tlensfree, Tq, Tt} <: BK.AbstractBoundaryValueProblem
	...
	"updates the section every `updateSectionEveryStep` step during continuation"
	updateEveryStep::Int
...
function HomoclinicHyperbolicProblemPBC(bvp::Tbvp,
	...
	HomoclinicHyperbolicProblemPBC{Tbvp, Nfree, Tlens, Ty, typeof(freeparams), typeof(Qs0), typeof(test0)}(bvp, lens, T, ϵ0, ϵ1, freeparams, Qu0, Qs0, N, updateEveryStep, jacobian, test0,testOrbitFlip, testInclinationFlip, nUnstable, nStable )
end
...
function Base.show(io::IO, hom::HomoclinicHyperbolicProblemPBC)
	...
	println(io, "├─ T           : ", hom.T)
	println(io, "├─ free params : ", map(BK.getLensSymbol,hom.freelens))
	println(io, "├─ update prob : ", hom.updateEveryStep)
	println(io, "└─ lens        : ", BK.getLensSymbol(BK.getLens(hom)))
...
function generateHomSolution(pb::BK.AbstractBoundaryValueProblem, orbit, T)
	M = getMeshSize(pb)
	orbitguess_a = [orbit(-T + t * (2T)) for t in LinRange(0, 1, M + 1)[1:M]]
	# append period at the end of the initial guess
	orbitguess_v = reduce(vcat, orbitguess_a)
	if pb isa AbstractPoincareShootingProblem
...
function getTestsforHHS(𝐇𝐨𝐦, J, z, xₛ, x₀, x₁, T::Ty, pars; tol = 1e-5) where Ty
	F = eigen(J)
...
function BK.continuation(𝐇𝐨𝐦::HomoclinicHyperbolicProblemPBC,
	...
	function updateHom(z, tau, step, contResult; kUP...)
		...
		if (~BK.modCounter(step, 𝐇𝐨𝐦.updateEveryStep) || success == false)
		...
		if success && (𝐇𝐨𝐦.bvp isa PeriodicOrbitOCollProblem) && 𝐇𝐨𝐦.bvp.meshadapt
			...
			oldsol = BK._copy(z)
			oldmesh = BK.getTimes(𝐇𝐨𝐦.bvp) .* getPeriod(𝐇𝐨𝐦.bvp, z.u, nothing)
			oldu = vcat(z.u.x[1], T)
			adapt = BK.computeError!(𝐇𝐨𝐦.bvp, oldu;
					verbosity = 𝐇𝐨𝐦.bvp.verboseMeshAdapt,
					par = newpar,
					K = 𝐇𝐨𝐦.bvp.K)
		...
		if success && BK.modCounter(step, 𝐇𝐨𝐦.updateEveryStep) == 1
			𝐇𝐨𝐦.Qs0 .= Qs0
			𝐇𝐨𝐦.Qu0 .= Qu0
	...
	function testHom(iter, state)
		...
		u = vcat(z.x[1], T)
		uc = BK.getTimeSlices(𝐇𝐨𝐦.bvp, u)
		x0 = @view uc[:, 1]
		x1 = @view uc[:, end]

		getTestsforHHS(𝐇𝐨𝐦, J, z, z.x[2], x0, x1, T, newpar) |> values
	end
	...
	probhom_bk = BifurcationProblem(𝐇𝐨𝐦, homguess, BK.getParams(𝐇𝐨𝐦), lens;
		...
		recordFromSolution = (x, p) -> begin
			if length(𝐇𝐨𝐦.freelens) == 1
				lensS = map(BK.getLensSymbol, (BK.getLens(𝐇𝐨𝐦), lens, @lens _.FreeP1))
			else
				lensS = map(BK.getLensSymbol, (BK.getLens(𝐇𝐨𝐦), lens, (@lens _.FreeP1), @lens _.FreeP2))
			end
			record = (;zip(lensS, (x.x[end][1], p, x.x[end][2:end]...))...)
			if _contParams.detectEvent > 0
				record = merge(record, 𝐇𝐨𝐦.test)
			end
			return record
		end,
		plotSolution = modifyHomPlot(𝐇𝐨𝐦, lens, kwargs),
		)
	...
	return BK.continuation(probhom_bk, alg, _contParams;
		finaliseSolution = 𝐇𝐨𝐦.updateEveryStep == 0 ? get(kwargs, :finaliseSolution, BK.finaliseDefault) : updateHom,
...
- `bt::BK.BogdanovTakens` a Bogdanov-takens point. For example, you can get this from a call to `bt = getNormalForm(br, ind_bt)`
...
function BK.continuation(prob_vf,
	...
	prob_vf = reMake(prob_vf, params = pars)
	...
	𝐇𝐨𝐦 = HomoclinicHyperbolicProblemPBC(bvp, lens1, length(xsaddle), copy(J);  ϵ0 = ϵ0hom, ϵ1 = ϵ1hom, T = Thom, freeparams = freeparams, updateEveryStep = updateEveryStep, testOrbitFlip = testOrbitFlip,
	testInclinationFlip = testInclinationFlip)

	@assert BK.getParams(𝐇𝐨𝐦) == pars "Errors with setting the parameters. Please an issue on the website of BifurcationKit."

	solsaddle = BK._newton(BifurcationProblem((x,p) -> getF(𝐇𝐨𝐦,x,p),xsaddle,nothing), xsaddle, pars, NewtonPar(verbose = true), norm = x->norm(x,Inf))
