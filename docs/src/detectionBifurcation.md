...
When continuing a homoclinic orbit, you can detect the following codimension-two bifurcation points by using the option `detect_codim2_bifurcation` in the method [`continuation`](@ref). Under the hood, the detection of these bifurcations is done by using Event detection as explained in [Event Handling](https://bifurcationkit.github.io/BifurcationKitDocs.jl/dev/EventCallback/).
